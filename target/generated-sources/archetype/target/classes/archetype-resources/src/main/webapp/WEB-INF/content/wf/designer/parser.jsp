...
function parseProcessDescriptor(data){
	...
	var shapes = descriptor.find('bpmndi${symbol_escape}${symbol_escape}:BPMNShape');
	var edges = descriptor.find('bpmndi${symbol_escape}${symbol_escape}:BPMNEdge');
	...
	if(extentsion != null){
		var listeners = extentsion.find('activiti${symbol_escape}${symbol_escape}:executionListener');
	...
	startEvent.each(function(i){
			var start = new draw2d.Start("${symbol_dollar}{ctx}/js/designer/icons/type.startevent.none.png");
			...
			shapes.each(function(i){
				...
				if(id==start.id){
					var x=parseInt(jq(this).find('omgdc${symbol_escape}${symbol_escape}:Bounds').attr('x'));
					var y=parseInt(jq(this).find('omgdc${symbol_escape}${symbol_escape}:Bounds').attr('y'));
					var width=parseInt(jq(this).find('omgdc${symbol_escape}${symbol_escape}:Bounds').attr('width'));
					var height=parseInt(jq(this).find('omgdc${symbol_escape}${symbol_escape}:Bounds').attr('height'));
	...
	endEvent.each(function(i){
			var end = new draw2d.End("${symbol_dollar}{ctx}/js/designer/icons/type.endevent.none.png");
	...
	userTasks.each(function(i){
			...
			var listeners = jq(this).find('extensionElements').find('activiti${symbol_escape}${symbol_escape}:taskListener');
	...
	exclusiveGateway.each(function(i){
			var gateway = new draw2d.ExclusiveGateway("${symbol_dollar}{ctx}/js/designer/icons/type.gateway.exclusive.png");
	...
	parallelGateway.each(function(i){
		var gateway = new draw2d.ExclusiveGateway("${symbol_dollar}{ctx}/js/designer/icons/type.gateway.parallel.png");
		var gtwid = jq(this).attr('id');
		var gtwname = jq(this).attr('name');
		gateway.id=gtwid;
		gateway.gatewayId=gtwid;
	...
	lines.each(function(i){
			...
			edges.each(function(i){
					...
					if(eid==lid){
						var startPort = null;
						var endPort = null;
						var points = jq(this).find('omgdi${symbol_escape}${symbol_escape}:waypoint');
...
function parseListeners(listeners,listenerType,fieldType){
	...
	listeners.each(function(i){
		...
		var fields = jq(this).find('activiti${symbol_escape}${symbol_escape}:field');
		fields.each(function(i){
			var field = eval("new "+fieldType+"()");
			field.name=jq(this).attr('name');
			//alert(field.name);
			var string = jq(this).find('activiti${symbol_escape}${symbol_escape}:string').text();
			var expression = jq(this).find('activiti${symbol_escape}${symbol_escape}:expression').text();
