...
<script>
	${symbol_dollar}(document).ready(function() {
			${symbol_dollar}("${symbol_pound}loginForm").validate({
				rules:{
					j_username:{
						remote:"${symbol_dollar}{ctx}/pm/user/user!checkUserIsExisting.action"
					}
				},
				messages:{
					j_username:{
			...
			${symbol_dollar}('${symbol_pound}logonPanel').panel({});
			${symbol_dollar}('${symbol_pound}j_password').bind('keyup',function(e){
					var keyCode = e.keyCode || e.charCode;
					if(keyCode==13){
						logon();
					}
				});
			${symbol_dollar}('${symbol_pound}j_username').focus();
		});
	function logon() {
		${symbol_dollar}('${symbol_pound}loginForm').submit();
	}
</script>
</head>
			...
			<tr>
				...
				<td align="center" valign="top" height="400">
					<div id="logonPanel" title="<s:text name='label.login.title'></s:text>" style="width:400px; height: 200px;">
						<form id="loginForm" action="${symbol_dollar}{ctx}/j_spring_security_check"
						method="post">
						<%
							String message = "";
						...
						<table>
							...
							<tr>
								...
								<td colspan='2' align="center"><a id="loginbt" href="${symbol_pound}${symbol_pound}${symbol_pound}" class="easyui-linkbutton" onclick="logon()"><s:text name="button.login.logon"></s:text></a></td>
							</tr>
							<tr>
								...
								<!-- 指定URL为中文的语言设置 -->   
				                 	...
				                 	<!-- 中文语言设置的链接定义 -->   
				                  	<s:a href="%{chinese}"><img src="${symbol_dollar}{ctx}/image/zh_CN.gif" border="0" alt="<s:text name="label.lang.zh"></s:text>"/></s:a>  
									<s:url id="english" action="login.action">   
				                         <!-- 参数request_locale设置英文 -->   
				                         <s:param name="request_locale">en_US</s:param>   
				                  	</s:url>   
				                  	<!-- 英文语言设置的链接定义 -->   
				                  	<s:a href="%{english}"><img src="${symbol_dollar}{ctx}/image/en_US.gif" border="0" alt="<s:text name="label.lang.us"></s:text>"/></s:a>    
				                </td>
							</tr>
						</table>
						</form>
