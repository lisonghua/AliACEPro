...
<!--
$(function(){
	_pw_editor_obj = $('#_pw_editor').panel({
		height:200
	});
	_pw_form_obj = $('#_pw_Form');
	_pw_form_obj.validate({
		rules: {
			opw:{
				required: true,
				remote: "${ctx}/pm/user/user!checkOldPassword.action?userId="+encodeURIComponent(document.getElementById('userId').value)+"&oldPassword=" + encodeURIComponent(document.getElementById('opw').value),
				maxlength:10
			},
			npw: {
				required: true,
...
function updatePw(){
	_pw_form_obj.form('submit',{
		url:'${ctx}/pm/user/user!updatePassword.action',
...
<body>
<div id="_pw_editor" style="background:#fafafa;padding:5px;">
	<form id="_pw_Form">
