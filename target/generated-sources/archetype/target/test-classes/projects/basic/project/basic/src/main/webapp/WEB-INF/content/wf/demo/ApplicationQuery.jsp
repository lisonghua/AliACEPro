...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		title:'',
		url:'${ctx}/wf/application/application!search.action',//加载表格数据的URL
		singleSelect:true,
		height:500,
		idField:'appId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	//点击添加按钮的URL
	    		window.location='${ctx}/wf/application/application!forInsert.action';
	    		return false;
	        }
	    }]
	});
...
//点击修改按钮转向的URL
function  editRow(id){
	window.location='${ctx}/wf/application/application!forUpdate.action?appId='+id;
}
//点击删除按钮的URL
function  deleteRow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/wf/application/application!delete.action?appId='+id,//删除调用的URL
					type: 'GET',
					dataType:'json',
					error:function(){
...
function searchByFilter(){
	var appId=document.getElementById("filter_EQI_appId").value;
	_list_grid_obj.datagrid('reload',{
		filter_EQI_appId:appId
	});
}
//-->
</script>
...
-->
	...
	<table border="0">
		<tr>
			<td>AppId:</td>
			<td><input type="text" name="filter_EQI_appId" value="" size="9"/></td>
...
<table id="_list_table">
	<thead>
		<tr>
			<th field="appId" width="100" align="middle" sortable="true">AppId</th>
			<th field="processInstanceId" width="100" align="middle" >ProcessInstanceId</th>
			<th field="applicant" width="100" align="middle" >Applicant</th>
			<th field="applyDate" width="100" align="middle" >ApplyDate</th>
			<th field="leaveType" width="100" align="middle" >LeaveType</th>
			<th field="startDate" width="100" align="middle" >StartDate</th>
			<th field="endDate" width="100" align="middle" >EndDate</th>
			<th field="reason" width="100" align="middle" >Reason</th>
