...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		title:'',
		url:'${ctx}/genpact/authorization/authorization!search.action',//加载表格数据的URL
		singleSelect:true,
		height:400,
		idField:'rpAutId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	//点击添加按钮的URL
	    		window.location='${ctx}/genpact/authorization/authorization!forInsert.action';
	    		return false;
	        }
	    }]
	});
...
//点击修改按钮转向的URL
function  editRow(id){
	window.location='${ctx}/genpact/authorization/authorization!forUpdate.action?rpAutId='+id;
}
//点击删除按钮的URL
function  deleteRow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/genpact/authorization/authorization!delete.action?rpAutId='+id,//删除调用的URL
					type: 'GET',
					dataType:'json',
					error:function(){
...
function searchByFilter(){
	var rpAutId=document.getElementById("filter_EQI_rpAutId").value;
	_list_grid_obj.datagrid('reload',{
		filter_EQI_rpAutId:rpAutId
	});
}
//-->
</script>
...
-->
	...
	<table border="0">
		<tr>
			<td>RpAutId:</td>
			<td><input type="text" name="filter_EQI_rpAutId" value="" size="9"/></td>
...
<table id="_list_table">
	<thead>
		<tr>
			<th field="rpAutId" width="100" align="middle" sortable="true">RpAutId</th>
			<th field="role" width="100" align="middle" >Role</th>
