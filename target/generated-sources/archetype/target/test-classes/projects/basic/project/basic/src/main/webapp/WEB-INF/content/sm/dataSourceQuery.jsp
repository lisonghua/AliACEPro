...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		...
		url:'${ctx}/sm/datasource/datasource!search.action',
		singleSelect:true,
		height:500,
		//idField:'serverId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
	    		window.location='${ctx}/sm/datasource/datasource!forUpdate.action';
	    		return false;
	        }
	    }]
	});
...
function  editRow(id){
	window.location='${ctx}/sm/datasource/datasource!forUpdate.action?dsId='+id;
}
function  deleteRow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/sm/datasource/datasource!delete.action?dsId='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
...
function analyzeLog(id){
	window.location='${ctx}/sm/datasource/datasource!forAnalyze.action?dsId='+id;
}
