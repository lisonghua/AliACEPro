...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
				window.open("${ctx}/wf/procdef/procdef!forDefinition.action","designer","top=0,left=0,width=1450px,height=750px,directories=0,menubar=0,resizable=0,status=0,titlebar=0,toolbar=0");
		    }
	    },'-',{
	...
	$('#importProcessDefXmlForm').ajaxForm({
		dataType:'xml',
		resetForm:true,
		success:function(responseText, statusText, xhr, jqForm){
			...
			if(success == "true"){
	            ...
	            $.messager.alert("<s:text name='label.common.success'></s:text>","<s:text name='message.wf.process.definition.import.success'></s:text>","info");
	    	}else{
	    		$.messager.alert("<s:text name='label.common.error'></s:text>","<s:text name='message.wf.process.definition.import.fail'></s:text>","error");
	        }
		}
...
function  editProcessDef(id,name){
	//window.showModalDialog("${ctx}/wf/procdef/procdef!forDefinition.action?procDefId="+id,window,"dialogHeight:600px;dialogWidth:800px;center:yes;help:no;scroll:no;");
	window.open("${ctx}/wf/procdef/procdef!forDefinition.action?procDefId="+id+"&procDefName="+name,"designer","top=0,left=0,width=1450px,height=750px,directories=0,menubar=0,resizable=0,status=0,titlebar=0,toolbar=0");
}
...
function exportProceDef(id,obj){
	obj.href="${ctx}/wf/procdef/procdef!exportProcessDef.action?procdefId="+id;
}
//输出每一行末尾的操作按钮
function actionFormatter(value,rowData,rowIndex){
	var id = rowData.id;
	var name = rowData.name;
	var e = '<img onclick="editProcessDef(\''+id+'\',\''+name+'\')" src="${ctx}/image/edit.gif" title="'+"<s:text name='button.common.modify'></s:text>"+'" style="cursor:hand;"/>';   
    var exp = '<a href="#" onclick="exportProceDef(\''+id+'\',this)"><img border="0px" src="${ctx}/image/download.png" title="'+"<s:text name='lable.wf.process.definition.export.bt.title'></s:text>"+'" style="cursor:hand;"/></a>';
    return e+'&nbsp;'+exp;
}
//点击查询按钮的操作函数
...
<div id="_wf_diagram_win" href="" title="<s:text name='lable.wf.process.definition.import.window.title'></s:text>" style="width:700px;height:100px;">  
   	<form id="importProcessDefXmlForm" action="${ctx}/wf/procdef/procdef!importProcessDef.action" method="post" enctype="multipart/form-data">
   		<table>
   			<tr>
