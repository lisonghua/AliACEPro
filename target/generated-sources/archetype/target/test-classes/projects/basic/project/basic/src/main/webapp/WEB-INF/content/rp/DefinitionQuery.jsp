...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		title:'',
		url:'${ctx}/genpact/definition/definition!search.action',//加载表格数据的URL
		singleSelect:true,
		height:400,
		idField:'rpDefId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	//点击添加按钮的URL
	    		window.location='${ctx}/genpact/definition/definition!forInsert.action';
	    		return false;
	        }
	    }]
	});
...
//点击修改按钮转向的URL
function  editRow(id){
	window.location='${ctx}/genpact/definition/definition!forUpdate.action?rpDefId='+id;
}
//点击删除按钮的URL
function  deleteRow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/genpact/definition/definition!delete.action?rpDefId='+id,//删除调用的URL
					type: 'GET',
					dataType:'json',
					error:function(){
...
function searchByFilter(){
	var rpDefId=document.getElementById("filter_EQI_rpDefId").value;
	_list_grid_obj.datagrid('reload',{
		filter_EQI_rpDefId:rpDefId
	});
}
//-->
</script>
...
-->
	...
	<table border="0">
		<tr>
			<td>RpDefId:</td>
			<td><input type="text" name="filter_EQI_rpDefId" value="" size="9"/></td>
...
<table id="_list_table">
	<thead>
		<tr>
			<th field="rpDefId" width="100" align="middle" sortable="true">RpDefId</th>
			<th field="name" width="100" align="middle" >Name</th>
			<th field="businessKey" width="100" align="middle" >BusinessKey</th>
			<th field="templateName" width="100" align="middle" >TemplateName</th>
			<th field="remark" width="100" align="middle" >Remark</th>
			<th field="parameters" width="100" align="middle" >Parameters</th>
			<th field="authorizations" width="100" align="middle" >Authorizations</th>
