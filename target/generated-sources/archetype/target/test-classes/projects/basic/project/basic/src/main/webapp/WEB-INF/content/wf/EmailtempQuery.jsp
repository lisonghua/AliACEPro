...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		...
		url:'${ctx}/wf/emailtemp/emailtemp!search.action',//加载表格数据的URL
		singleSelect:true,
		height:500,
		idField:'tid',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	//点击添加按钮的URL
	    		window.location='${ctx}/wf/emailtemp/emailtemp!forInsert.action';
	    		return false;
	        }
	    }]
	});
...
//点击修改按钮转向的URL
function  editRow(id){
	window.location='${ctx}/wf/emailtemp/emailtemp!forUpdate.action?tid='+id;
}
//点击删除按钮的URL
function  deleteRow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/wf/emailtemp/emailtemp!delete.action?tid='+id,//删除调用的URL
					type: 'GET',
					dataType:'json',
					error:function(){
