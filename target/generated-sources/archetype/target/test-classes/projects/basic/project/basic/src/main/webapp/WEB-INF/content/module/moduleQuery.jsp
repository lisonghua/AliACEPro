...
<script type="text/javascript">
	$(function() {
		_function_panel_obj=$('#_function_panel').panel({
			//height:600,
			border:false,
			noheader:true,
			top:0,
		...
		_menu_table_obj = $('#_menu_table').treegrid(
						{
		...
		//							    	    } else {
													...
													if(id!=0){
										    	        ...
										    	        var a = '<img onclick="addfields('+id+')" src="${ctx}/image/delegate.gif" title="'+"<s:text name='label.pm.module.add.field'></s:text>"+'" style="cursor:hand;"/>';
										    	        var p = '<img onclick="assignPrivilege('+id+',\''+text+'\')" src="${ctx}/image/permissions.gif" title="'+"<s:text name='label.pm.module.privilege'></s:text>"+'" style="cursor:hand;"/>';
														if(funOrMod==1)
										    	        	return e+'&nbsp;'+a+'&nbsp;'+p+'&nbsp;'+d;
										    	        else
										    	        	return e+'&nbsp;'+d;
							...
							toolbar:[
							         {
							        	text:"<s:text name='button.common.add'></s:text>",
							 	        iconCls:'icon-add',
							 	        handler:function(){
							 	    		window.location='${ctx}/pm/module/module!forInsert.action';
							 	    		return false;
							 	        }
								     }	
							]	
    ...
    function  editrow(id){
    	window.location='${ctx}/pm/module/module!forInsert.action?moduleId='+id;
    }
	function  deleterow(id){
		$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
			if(flag){
				$.ajax({
						url:'${ctx}/pm/module/module!delete.action?moduleId='+id,
						type: 'GET',
						dataType:'json',
						error:function(){
    ...
    function addfields(id){
    	...
    	_field_frame_obj.src='${ctx}/pm/field/field!forQuery.action?moduleId='+id;
    }
    function assignPrivilege(id,name){
        //alert('${ctx}/pm/module/module!forAssignPrivilege.action?moduleId='+id+'&moduleName='+name);
    	window.location='${ctx}/pm/module/module!forAssignPrivilege.action?moduleId='+id;
    }
    function getDicValueByCode(parent,code){
    	$.ajax({
  			url:'${ctx}/common/dic/dic!getValueByCode.action?_dic_parent='+parent+'&_dic_code='+code,
			type: 'GET',
			dataType:'json',
			error:function(){
