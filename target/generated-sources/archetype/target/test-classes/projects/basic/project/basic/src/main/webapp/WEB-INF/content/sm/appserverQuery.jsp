...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		...
		url:'${ctx}/sm/appserver/appserver!search.action',
		singleSelect:true,
		height:500,
		//idField:'serverId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
	    		window.location='${ctx}/sm/appserver/appserver!forUpdate.action';
	    		return false;
	        }
	    }]
	});
...
function  editRow(id){
	window.location='${ctx}/sm/appserver/appserver!forUpdate.action?serverId='+id;
}
function  deleteRow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/sm/appserver/appserver!delete.action?serverId='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
...
function actionFormatter(value,rowData,rowIndex){
	...
	var c = '<img onclick="analyzeLog('+id+')" src="${ctx}/image/chart_bar_edit.png" title="'+"<s:text name='button.sm.ds.analyze.log'></s:text>"+'" style="cursor:hand;"/>';
	var u = '<img onclick="uploadLog('+id+')" src="${ctx}/image/upload.png" title="'+"<s:text name='button.sm.ds.upload.log'></s:text>"+'" style="cursor:hand;"/>';
    return u+'&nbsp;'+c+'&nbsp;'+e+'&nbsp;'+d;
}
function analyzeLog(id){
	window.location='${ctx}/sm/appserver/appserver!forAnalyze.action?serverId='+id;
}
function uploadLog(id){
	window.location='${ctx}/sm/appserver/appserver!forUpload.action?serverId='+id;
}
//-->
</script>
<body>
