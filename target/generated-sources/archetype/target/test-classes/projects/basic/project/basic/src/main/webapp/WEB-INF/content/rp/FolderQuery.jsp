...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		title:'',
		url:'${ctx}/genpact/folder/folder!search.action',//加载表格数据的URL
		singleSelect:true,
		height:400,
		idField:'rpFolderId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	//点击添加按钮的URL
	    		window.location='${ctx}/genpact/folder/folder!forInsert.action';
	    		return false;
	        }
	    }]
	});
...
//点击修改按钮转向的URL
function  editRow(id){
	window.location='${ctx}/genpact/folder/folder!forUpdate.action?rpFolderId='+id;
}
//点击删除按钮的URL
function  deleteRow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/genpact/folder/folder!delete.action?rpFolderId='+id,//删除调用的URL
					type: 'GET',
					dataType:'json',
					error:function(){
