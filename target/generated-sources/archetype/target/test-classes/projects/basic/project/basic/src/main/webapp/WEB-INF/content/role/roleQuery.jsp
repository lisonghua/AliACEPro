...
<!--
$(function(){
	_role_panel_obj=$('#_role_panel').panel({
			//height:600,
			border:false,
			noheader:true,
			top:0,
	...
	_role_grid=$('#_role_table').datagrid({
		title:"<s:text name='label.pm.role.list'></s:text>",
		url:'${ctx}/pm/role/role!search.action',
		//singleSelect:true,
		height:500,
		idField:'roleId',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        iconCls:'icon-add',
	        handler:function(){
	    		window.location='${ctx}/pm/role/role!forInsert.action';
				return false;
	        }
	    }]
	});
...
function  editrow(id){
	window.location='${ctx}/pm/role/role!forUpdate.action?roleId='+id;
}
function  deleterow(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/pm/role/role!delete.action?_role_id='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
...
function displayActionBt (value,rowData,rowIndex){
        ...
        var a = '<img onclick="addUsers('+id+')" src="${ctx}/image/assign.gif" title="'+"<s:text name='button.pm.role.add.user'></s:text>"+'" style="cursor:hand;"/>';
        var p = '<img onclick="assignPrivilege('+id+')" src="${ctx}/image/permissions.gif" title="'+"<s:text name='button.pm.role.assign.privilege'></s:text>"+'" style="cursor:hand;"/>';
        return e+'&nbsp;'+a+'&nbsp;'+p+'&nbsp;'+d;
}
function addUsers(id){
	//alert(id);
	window.location="${ctx}/pm/role/role!forAssignUser.action?roleId="+id;
}
function assignPrivilege(id){
    //alert('${ctx}/pm/module/module!forAssignPrivilege.action?moduleId='+id+'&moduleName='+name);
	window.location='${ctx}/pm/role/role!forAssignPrivilege.action?roleId='+id;
}
//-->
</script>
<body>
