...
$(function(){
	...
	_field_grid_obj=$('#_field_table').datagrid({
			url : '${ctx}/pm/role/role!searchFieldsOfModule.action?roleId=<%=request.getParameter("roleId")%>&moduleId=<%=request.getParameter("moduleId")%>',
...
function savePrivilege(){
	...
	$.ajax({
			url:"${ctx}/pm/role/role!savePrivilegeOfField.action",
		type: 'POST',
		data:{
					roleId:"${roleId}",
