...
$(function(){
	...
	_list_grid_obj=$('#_list_table').datagrid({
		...
		url:'${ctx}/wf/procinst/procinst!search.action?procdefId=${id}',//加载表格数据的URL
		singleSelect:true,
		height:500,
		idField:'id',
		pagination:true,
	    ...
	    toolbar:[{
	        ...
	        handler:function(){
	        	//点击添加按钮的URL
	    		window.location='${ctx}/wf/procdef/procdef!forQuery.action';
	    		return false;
	        }
	    }]
	});
	...
	_wf_hitory_win_obj = $('#_wf_hitory_win').window({   
	    closed:true,
	    draggable:false,
	    collapsible:false,
	    minimizable:false,
	...
	_wf_hitory_frame_obj = $('#_wf_hitory_frame');
	var _filter_start_date=$('#filter_GED_startTime').datebox({
		formatter:simpleDateFormatter,
		parser:simpleDateParser
		});
	var _filter_end_date=$('#filter_LED_endTime').datebox({
		formatter:simpleDateFormatter,
		parser:simpleDateParser
		});
});
...
function  viewDiagram(id){
	...
	//_wf_diagram_win_obj.window('refresh','${ctx}/wf/procinst/procinst!forDiagram.action?procdefId=${id}&id='+id);
	window.open("${ctx}/wf/procinst/procinst!forDiagram.action?procdefId=${id}&id="+id,"diagram","width=1500px,height=800px,directories=0,menubar=0,resizable=0,status=0,titlebar=0,toolbar=0");
}
//点击浏览流程图按钮的URL
function  viewHistory(id){
	...
	//_wf_hitory_win_obj.window('refresh','${ctx}/wf/procinst/procinst!forHistory.action?procdefId=${id}&id='+id);
	_wf_hitory_frame_obj.attr({src:"${ctx}/wf/procinst/procinst!forHistory.action?procdefId=${id}&id="+id});
}
function killInstance(id){
	$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
		if(flag){
			$.ajax({
					url:'${ctx}/wf/procinst/procinst!delete.action?id='+id,
					type: 'GET',
					dataType:'json',
					error:function(){
...
function actionFormatter(value,rowData,rowIndex){
	var id = rowData.id;
	var v = '<img onclick="viewHistory(\''+id+'\')" src="${ctx}/image/zoom.png" title="'+"<s:text name='button.wf.process.history.view'></s:text>"+'" style="cursor:hand;"/>';   
    var vd = '<img onclick="viewDiagram(\''+id+'\')" src="${ctx}/image/picture_go.png" title="'+"<s:text name='button.wf.process.diagram.view'></s:text>"+'" style="cursor:hand;"/>';
    var k = '<img onclick="killInstance(\''+id+'\')" src="${ctx}/image/cancel.png" title="'+"<s:text name='button.wf.process.instance.cancel'></s:text>"+'" style="cursor:hand;"/>';
    return v+'&nbsp;'+vd+"&nbsp;"+k;
}
//点击查询按钮的操作函数
function searchByFilter(){
	var id=document.getElementById("filter_EQS_procInstId").value;
	var start= $('#filter_GED_startTime').datebox("getValue");
	var end=$('#filter_LED_endTime').datebox("getValue");
	_list_grid_obj.datagrid('reload',{
