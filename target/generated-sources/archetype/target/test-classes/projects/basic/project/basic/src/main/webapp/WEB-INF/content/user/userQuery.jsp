...
<!--
$(function(){
	_user_panel_obj=$('#_user_panel').panel({
			//height:600,
			//fit:true,
			border:false,
			noheader:true,
	...
	_users_grid=$('#userTable').datagrid({
		title:'<s:text name="label.pm.user.table.title"></s:text>',
		url:'${ctx}/pm/user/user!searchUser.action',
		//singleSelect:true,
		height:500,
		//fit:true,
		idField:'userId',
...
function addUser(){
	window.location='${ctx}/pm/user/user!forInsert.action';
	return false;
}
function dosearch(){
...
function updateUser(){
	...
	if(rows.length==0){
		...
		$.messager.alert('Error',"<s:text name='message.common.only.selectOneRow'></s:text>",'error');
		return false;
	}else{
		var seleted = _users_grid.datagrid("getSelected");
		//alert('${ctx}/pm/user/user!forUpdate.action?userId='+seleted.userId);
		window.location='${ctx}/pm/user/user!forUpdate.action?userId='+seleted.userId;
		return false;
	}
}
function deleteUser(){
	...
	if(rows.length==0){
		...
		$.messager.confirm("Confirm","<s:text name='message.common.delete.confirm'></s:text>",function(flag){
			...
			if(flag){
				...
				//alert(ids);
				$.ajax({
						url:'${ctx}/pm/user/user!delete.action?ids='+ids,
						type: 'GET',
						dataType:'json',
						error:function(){
...
function actionFormatter(value,rowData,rowIndex){
	...
	var p = '<img onclick="assignPrivilege('+id+')" src="${ctx}/image/permissions.gif" title="'+"<s:text name='button.pm.user.set.privilege'></s:text>"+'" style="cursor:hand;"/>';
	var sp = '<img onclick="setPassword('+id+')" src="${ctx}/image/setPassword.gif" title="'+"<s:text name='button.pm.user.update.password'></s:text>"+'" style="cursor:hand;"/>';
	var a = '<img onclick="activateUser('+id+')" src="${ctx}/image/activate.gif" title="'+"<s:text name='button.pm.user.activate'></s:text>"+'" style="cursor:hand;"/>';
	var da = '<img onclick="deactivateUser('+id+')" src="${ctx}/image/deactivate.gif" title="'+"<s:text name='button.pm.user.deactivate'></s:text>"+'" style="cursor:hand;"/>';
	var r = '<img onclick="assignRole('+id+')" src="${ctx}/image/role.gif" title="'+"<s:text name='button.pm.user.assign.role'></s:text>"+'" style="cursor:hand;"/>';
	//super user
	if(id==0)
		return sp;
	else if(ustatus==0)
...
function activateUser(id){
	$.ajax({
		url:'${ctx}/pm/user/user!activateUser.action?userId='+id,
		type: 'GET',
		dataType:'json',
		error:function(){
			$.messager.alert('Error',"<s:text name='message.pm.user.activate.failure'></s:text>",'error');
		},
		success:function(data){
			if(data.success){
				$.messager.alert('Success',data.message,'info');
...
function deactivateUser(id){
	$.ajax({
		url:'${ctx}/pm/user/user!deactivateUser.action?userId='+id,
		type: 'GET',
		dataType:'json',
		error:function(){
			$.messager.alert('Error',"<s:text name='message.pm.user.deactivate.failure'></s:text>",'error');
		},
		success:function(data){
			if(data.success){
				$.messager.alert('Success',data.message,'info');
...
function assignPrivilege(id){
    //alert('${ctx}/pm/module/module!forAssignPrivilege.action?moduleId='+id+'&moduleName='+name);
	window.location='${ctx}/pm/user/user!forAssignPrivilege.action?userId='+id;
}
function assignRole(id){
    //alert('${ctx}/pm/module/module!forAssignPrivilege.action?moduleId='+id+'&moduleName='+name);
	window.location='${ctx}/pm/user/user!forAssignRole.action?userId='+id;
}
function setPassword(id){
    //alert('${ctx}/pm/module/module!forAssignPrivilege.action?moduleId='+id+'&moduleName='+name);
	//window.location='${ctx}/pm/user/user!forAssignPrivilege.action?userId='+id;
	_pw_window.window('open');
	_pw_frame.src="${ctx}/pm/user/user!forUpdatePassword.action?userId="+id;
}
