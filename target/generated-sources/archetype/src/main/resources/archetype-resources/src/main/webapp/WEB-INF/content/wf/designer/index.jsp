...
jq(function(){
	try{
		...
		_designer = jq('${symbol_pound}designer');
		...
		_task_context_menu = jq('${symbol_pound}task-context-menu').menu({});
		...
		jq('${symbol_pound}paintarea').droppable({
		...
		//jq('${symbol_pound}paintarea').bind('contextmenu',function(e){
...
function openProcessProperties(id){
	...
	if(!is_open_properties_panel)
		_designer.layout('expand','east');
	_properties_panel_obj.panel('refresh','${symbol_dollar}{ctx}/wf/procdef/procdef!forProcessProperties.action?processId='+id);
}
function openFlowProperties(id){
	//alert(id);
	if(!is_open_properties_panel)
		_designer.layout('expand','east');
	_properties_panel_obj.panel('refresh','${symbol_dollar}{ctx}/wf/procdef/procdef!forLineProperties.action?flowId='+id);
}
...
function saveProcessDef(){
	...
	//return;
	jq.ajax({
		url:"${symbol_dollar}{ctx}/wf/procdef/procdef!saveProcessDescriptor.action",
		type: 'POST',
		data:{
...
function exportProcessDef(obj){
	obj.href="${symbol_dollar}{ctx}/wf/procdef/procdef!exportProcessDef.action?procdefId="+processDefinitionId+"&processName="+processDefinitionName;
}
//-->
</script>
...
<!--					<a href="${symbol_pound}${symbol_pound}" class="easyui-linkbutton" plain="true" iconCls="sequence-flow-icon">SequenceFlow</a><br>-->
<!--				</div>-->
				<div id="event" title="Event" iconCls="palette-menu-icon" class="palette-menu">
					<a href="${symbol_pound}${symbol_pound}" class="easyui-linkbutton" plain="true" iconCls="start-event-icon">Start</a><br>
					<a href="${symbol_pound}${symbol_pound}" class="easyui-linkbutton" plain="true" iconCls="end-event-icon">End</a><br>
				</div>
				...
				<div id="boundary-event" title="Boundary event" iconCls="palette-menu-icon" class="palette-menu">
					...
					<a href="${symbol_pound}${symbol_pound}" class="easyui-linkbutton" plain="true" iconCls="error-boundary-event-icon">ErrorBoundaryEvent</a><br>
				</div>
		</div>
	</div>
	...
	<!-- toolbar -->
	<div id="toolbar-panel" region="north" border="false" style="height:36px;background:${symbol_pound}E1F0F2;">
		<div style="background:${symbol_pound}E1F0F2;padding:5px;">
			<a href="javascript:void(0)" id="sb1" class="easyui-splitbutton" menu="${symbol_pound}edit-menu" iconCls="icon-edit">Edit</a>
<!--			<a href="javascript:void(0)" id="sb2" class="easyui-splitbutton" menu="${symbol_pound}mm2" iconCls="icon-ok" onclick="javascript:alert(workflow.toXML())">Ok</a>-->
<!--			<a href="javascript:void(0)" id="mb2" class="easyui-menubutton" menu="${symbol_pound}xml/diagram" onclick="viewXMLOrDiagram()">XML/Diagram</a>-->
			<a href="javascript:void(0)" id="mb3" class="easyui-menubutton" menu="${symbol_pound}mm3" iconCls="icon-help">Help</a>
		</div>
...
<!--		<div onclick="openProcessDef()">Open</div>-->
		...
		<div><a href="${symbol_pound}" onclick="exportProcessDef(this)">Export</a></div>
<!--		<div class="menu-sep"></div>-->
<!--		<div>-->
<!--			<span>Toolbar</span>-->
